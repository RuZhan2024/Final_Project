...
function hasStoredSkeletonClip(event) {
  return Boolean(event?.meta?.skeleton_clip?.path);
}

export function EventSkeletonClipPanel({ apiBase, event, residentId = 1 }) {
  ...
  useEffect(() => {
    ...
    async function loadClip() {
      ...
      try {
        ...
        setClipError(String(e?.message || e));
      } finally {
  ...
  const clipSavedAt = useMemo(
    () => (clipData?.clip?.saved_at ? parseDateSafe(clipData.clip.saved_at) : null),
    [clipData]
  );
