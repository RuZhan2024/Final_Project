...
import type { LegacyOperatingPointsResponse } from "../../../features/monitor/types";
...
async function loadLegacyOperatingPoint(
  ...
  for (const code of tryCodes) {
    try {
      const data = (await fetchOperatingPoints(
        apiBase,
        code,
        datasetCode
      )) as LegacyOperatingPointsResponse | unknown[];
      const ops = Array.isArray(data)
        ? data
        : data && typeof data === "object" && Array.isArray(data.operating_points)
...
export function useOperatingPointParams({
  ...
  const [legacy, setLegacy] = useState<{ tau_low: number | null; tau_high: number | null; op_code: string | null }>({
    tau_low: null,
    tau_high: null,
    op_code: null,
  });
