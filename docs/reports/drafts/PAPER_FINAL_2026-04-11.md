...
1. It presents a deployment-oriented pose-based monitoring system linking preprocessing, temporal inference, operating-point fitting, replay/live monitoring, persistent event handling, and caregiver-facing delivery within one audited pipeline.
...
## ML Pipeline Selection Logic

The repository preserves a broad ML history, including multiple optimisation families, sweep outputs, side tracks, and later retraining continuations. The paper does not attempt to represent that whole history evenly. Instead, it promotes only the ML work that materially sharpens the manuscript’s main line. In practice, that means the paper keeps four ML-facing components in focus: the locked offline comparison, the calibration-aware operating-point layer, the cross-dataset failure boundary, and the later targeted retraining that modestly improved the bounded `CAUCAFall + TCN` runtime surfaces. Other material, including the full sweep archaeology, temporary recheck files, and most exploratory side-track detail, is retained only as supporting or archival evidence.

This filtering is methodological rather than cosmetic. It prevents the manuscript from turning into an experiment diary and keeps the contribution centred on the strongest defended claim: a deployment-oriented pose-based monitoring system supported by controlled offline comparison and bounded runtime strengthening, rather than a broad catalogue of every training run executed during the project.
...
## Frontend/Backend Work Worth Promoting

The paper does not need the full implementation history, but several frontend/backend decisions are part of the deployment-oriented contribution. The first is the explicit split between browser-side window production and backend-side policy interpretation. This matters because some runtime behaviour was shaped not by checkpoint choice alone, but by the contract between pose extraction, window production rate, and backend decision logic. The second is the separation of replay and realtime semantics. Replay is treated as a bounded validation and demonstration path, whereas realtime is the event-producing path whose outputs can be persisted, reviewed, and delivered onward. The third is the event-to-review-to-notification chain itself. Persisted events, event history, and Telegram-first delivery together show that the system reached a genuine end-to-end monitoring state rather than stopping at monitor-visible scores.

These points are worth promoting because they are not generic engineering effort. They explain why deployment behaviour in this project must be read as a full-stack systems property rather than as a direct alias for offline model quality.
...
The manuscript deliberately promotes only the parameter adjustments that change system behaviour in interpretable ways. In practice, that means threshold pairs, smoothing, `k/n` persistence, cooldown, and guard logic, because these parameters determine whether a high-scoring window becomes a reviewable incident or is suppressed as transient noise. The exact fitted values are project-specific, but the adjustment logic is general: deployment-oriented tuning should be analysed through its effects on recall, false alerts, delay, and event semantics rather than treated as a generic threshold sweep.
...
It is also important not to flatten the deployment-oriented replay line and the fixed custom replay line into one score. The stronger historical `23/24` result belongs to a bounded online replay path used to show that the monitored deployment profile could perform strongly under a specific runtime setup. The later `15/24` baseline and `16/24` strengthening results belong to fixed custom replay surfaces retained to expose remaining false negatives, false positives, and policy trade-offs. They therefore answer different questions: one about bounded deployment feasibility, the other about bounded runtime failure structure.
...
The subset pattern also suggests a system-level camera-geometry effect. The corridor camera is mounted higher than the kitchen camera, while the remaining false negatives cluster in views where the subject is very close to the camera. In those closer views, the person is more likely to be only partially framed, perspective distortion is stronger, and the skeleton track is less stable. This is a plausible explanation for why the strengthening runs improved the corridor fall clips while leaving the kitchen fall subset largely unchanged: the runtime pose geometry in the kitchen clips appears to diverge more strongly from the geometry represented in the training scenes. One remaining kitchen miss (`kitchen_front_2`) also suggests a temporal-persistence issue rather than pure invisibility: the subject stands up from the sofa and then falls forward very rapidly, which is consistent with a strong instantaneous fall score that still fails to persist long enough under the stricter `OP-2` event logic.
...
RQ1 asked how the TCN and the custom GCN compare under the locked offline protocol. The evidence supports a cautious directional answer: the TCN trends stronger than the matched GCN on the primary `CAUCAFall` protocol and also remains stronger on the in-domain `LE2i` comparison. The current five-seed inferential budget does not justify a strong claim of definitive statistical superiority under the primary Wilcoxon analysis. The correct answer is therefore directional advantage under bounded statistical certainty, not final proof that TCN is universally better.

## Answer to RQ2

RQ2 asked whether validation-side calibration and operating-point fitting materially affect practical alerting. The answer is yes. The repository’s deployable behaviour depends on fitted operating-point profiles, not on raw single-window probabilities alone. Temperature scaling during operating-point fitting, together with policy parameters such as smoothing, `k/n`, and cooldown, shows that the alerting path is explicitly calibrated as an operational decision layer. This is an important systems contribution because it moves the project beyond a benchmark-only classifier framing and makes the final monitoring behaviour interpretable in deployment terms.

## Answer to RQ3

RQ3 asked what replay deployment evidence and limited realtime validation reveal about practical feasibility and runtime limits. The answer is mixed but useful. Replay-oriented deployment evidence is strong enough to support a practical system claim in controlled conditions, and the bounded realtime evidence chain in Figure 7 and Supplementary Video S1 further shows that the defended profile can support monitor-visible runtime behaviour and persisted Event History state in a live run. At the same time, the replay artifacts make clear that deployment behaviour is not uniform across datasets and profiles. On the custom replay surface, the preferred `TCN + OP-2` preset is more selective but miss-prone, while `TCN + OP-1` recovers more falls only by becoming much noisier on ADL clips; the matched `GCN + OP-2` profile is less selective still. The strengthening runs improve this picture modestly: the best retrained continuations raise the defended bounded replay surfaces to `16/24` without worsening ADL false positives, but they do not remove the need for careful policy fitting or solve all subset-level misses. The remaining kitchen weakness is consistent with reduced pose observability in closer, lower-mounted views, where subjects are more likely to be partially framed and pose tracks less stable than in the corridor setup. The uncertainty-aware MC path also does not improve the fixed replay matrix. Runtime behaviour therefore remains sensitive to policy choice, path-specific latency, frontend pose quality, and camera placement. The system demonstrates practical feasibility in controlled deployment settings, but not broad field closure or uniform runtime robustness.
...
The fourth limitation is deployment closure. Replay-oriented deployment evidence is much stronger than field or live evidence. The current field-validation pack is too small to support broad real-world conclusions, and realtime results remain bounded by environment and runtime constraints. Even within replay, the bounded runtime matrix supports a defended demo preset rather than a uniform deployment-success claim across all dataset-model-operating-point combinations. The custom replay surface is especially clear on this point: more fall capture can be bought by materially increasing ADL false alarms, so runtime quality depends on the trade-off one is willing to defend. The strengthening runs improve the defended TCN custom surfaces, but only modestly, and they still depend on confirm-disabled `fit_ops` fallback. In addition, the remaining kitchen weakness suggests that camera placement and close-range views materially affect pose observability: subjects nearer the camera appear more likely to be partially framed and to produce unstable skeletons than in the corridor setup, and at least one residual miss (`kitchen_front_2`) is also consistent with a very rapid forward collapse that fails the stricter persistence logic of `OP-2`. That means the project can now claim bounded deployment improvement under retraining, but not a solved deployment profile.
