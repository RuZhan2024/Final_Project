...
The subset pattern also suggests a system-level camera-geometry effect. In the corridor setup, the camera is higher (`~2.2m`) and the subject is typically farther away (`~3m`), yielding a more stable whole-body view. In the kitchen setup, the camera is lower (`~2.0m`) and the subject is often much closer (`~1–1.5m`). At that shorter distance, the person is more likely to be only partially framed, perspective distortion is stronger, and the skeleton track is less stable. This is a plausible explanation for why the strengthening runs improved the corridor fall clips while leaving the kitchen fall subset largely unchanged: the runtime pose geometry in the kitchen clips appears to diverge more strongly from the geometry represented in the training scenes. One remaining kitchen miss (`kitchen_front_2`) also suggests a temporal-persistence issue rather than pure invisibility: the subject stands up from the sofa and then falls forward very rapidly, which is consistent with a strong instantaneous fall score that still fails to persist long enough under the stricter `OP-2` event logic.
