...
| Delivery replay pack | [delivery_tcn_r2_train_hneg_op2_metrics.json](/Users/ruzhan/computer_science/Goldsmiths/Final_Project/fall_detection_v2/artifacts/fall_test_eval_20260315/delivery_tcn_r2_train_hneg_op2_metrics.json) | `TP=12`, `TN=12`, `FP=0`, `FN=0` on the 24-video delivery set | Strong bounded replay/deployment evidence |
...
The delivery-style replay pack provides stronger engineering evidence than anecdotal UI observations alone. The metrics file [delivery_tcn_r2_train_hneg_op2_metrics.json](/Users/ruzhan/computer_science/Goldsmiths/Final_Project/fall_detection_v2/artifacts/fall_test_eval_20260315/delivery_tcn_r2_train_hneg_op2_metrics.json) records a 24-video delivery pack with `12/12` fall clips and `12/12` non-fall clips correctly handled under a replay-oriented configuration. This result is useful, but its interpretive boundary is critical: it supports deployment/demo validation only and must not be presented as equivalent to unseen-test generalisation. The correct reading is that the runtime path can express the locked profile cleanly in a bounded delivery setting.
