...
Deployment evidence is strongest in the locked replay and delivery-style path. The deployment-lock validation report in [deployment_lock_validation.md](/Users/ruzhan/computer_science/Goldsmiths/Final_Project/fall_detection_v2/artifacts/reports/deployment_lock_validation.md) confirms that the locked runtime path uses the expected `TCN`, dataset `caucafall`, and operating point `OP-2`, with a fall threshold around `0.71`. The same report records successful manual replay checks for one non-fall replay and two fall replay clips.

**Table 5. Deployment and runtime evidence summary**

| Evidence slice | Source artifact | Main result | Reporting role |
| --- | --- | --- | --- |
| Deployment lock validation | [deployment_lock_validation.md](/Users/ruzhan/computer_science/Goldsmiths/Final_Project/fall_detection_v2/artifacts/reports/deployment_lock_validation.md) | Locked `TCN + caucafall + OP-2` path validated; replay spot checks passed | Confirms the deployed runtime profile |
| Four-folder custom replay set under unified profile | [unified_tcn_caucafall_op2_metrics.json](/Users/ruzhan/computer_science/Goldsmiths/Final_Project/fall_detection_v2/artifacts/fall_test_eval_20260330/unified_tcn_caucafall_op2_metrics.json) | `TP=3`, `TN=10`, `FP=2`, `FN=9` on the 24 labeled custom clips | Supporting consistency check only |
...
The four-folder custom replay set is still useful, but under a single unified policy source it no longer behaves like a perfect delivery-only package test. The metrics file [unified_tcn_caucafall_op2_metrics.json](/Users/ruzhan/computer_science/Goldsmiths/Final_Project/fall_detection_v2/artifacts/fall_test_eval_20260330/unified_tcn_caucafall_op2_metrics.json) records `TP=3`, `TN=10`, `FP=2`, and `FN=9` on the 24 labeled clips when evaluated under the same canonical `CAUCAFall TCN OP-2` profile used by the monitor. This result is therefore more valuable as a consistency and boundary check than as a headline success case. It reinforces the distinction between bounded custom replay checks and the primary offline evidence.
...
RQ3 asked what replay deployment evidence and limited realtime validation reveal about practical feasibility and runtime limits. The answer is mixed but useful. Replay-oriented deployment evidence is strong enough to support a practical system claim in bounded conditions, especially for the locked TCN `OP-2` runtime path. However, field evidence remains small and runtime behaviour is still sensitive to path-specific latency and frontend pose quality. The system therefore demonstrates practical feasibility in controlled deployment settings, but not broad field closure.
...
3. Extend the uncertainty and calibration path so that deployment-time probability interpretation and alert-policy calibration are even more tightly aligned.
