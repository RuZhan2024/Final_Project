...
This report presents a research-led, full-stack study of pose-based fall detection under a bounded and explicitly controlled evidence framework. It treats the project as an end-to-end pose-based fall-detection and monitoring system, supported by controlled model comparison and deployment-oriented evaluation. Fall detection is investigated not only as a window-level classification task, but as a monitoring problem that also requires alert-policy design, runtime interpretation, event persistence, and caregiver-facing delivery.
...
## Parameter Adjustment and Interpretation

The report does not attempt to catalogue every sweep or intermediate YAML revision. What matters is the subset of parameter adjustments that materially changes runtime behaviour and therefore affects interpretation. In practice, the important parameters are threshold pairs such as `tau_high` and `tau_low`, smoothing strength such as `ema_alpha`, persistence logic such as `k/n`, cooldown behaviour, and any guard logic that suppresses or promotes borderline events. These are worth discussing because they determine whether a high-scoring window becomes a reviewable incident or is treated as transient noise.

The rationale for discussing parameter adjustment is therefore methodological rather than documentary. Lower thresholds or looser persistence can recover more falls, but may do so by increasing ADL false alarms. Higher thresholds, stronger smoothing, or stricter persistence can reduce false positives, but may also suppress short or borderline falls. The project’s policy layer is strongest when these adjustments are read through their operational effects on recall, false-alert burden, delay, and event semantics rather than as isolated numbers.

This distinction also clarifies what is and is not generalisable. The exact fitted values are project-specific because they depend on the dataset mix, pose quality, runtime path, and alert-cost assumptions. By contrast, the adjustment logic is general. A deployment-oriented pose-monitoring system should analyse parameter changes in terms of their behavioural trade-offs, not treat threshold selection as a generic sweep whose internal meaning does not matter.
...
These runtime numbers should not be read as competing scores from one identical evaluation surface. Earlier deployment-oriented replay results, including the historical `23/24` line, came from a bounded online replay path used to show that the monitored deployment profile could perform strongly under a specific runtime setup. The later `15/24` and strengthened `16/24` lines refer instead to the fixed custom replay surfaces retained to analyse residual false negatives, false positives, and policy trade-offs. The former therefore functions as bounded deployment-feasibility evidence, while the latter functions as bounded runtime failure-analysis evidence.
...
This chapter answers the three research questions in light of the report’s full evidence hierarchy. The project’s contribution is strongest when those answers are kept at the right level of authority: stronger where frozen comparison and bounded runtime evidence are clear, and more cautious where inferential or deployment limits remain.

## Answer to RQ1

`RQ1` asked how the TCN and the custom spatio-temporal GCN compare under the locked offline protocol. The answer is cautious but directionally clear. Under the frozen primary-dataset protocol, the TCN trends stronger than the matched GCN, and the same directional preference also appears in the in-domain `LE2i` comparison. The present inferential budget does not justify stronger language about universal architectural superiority, so the correct interpretation is directional advantage under bounded statistical certainty.

That result remains important after the system chapters are taken into account. Replay, audit, and runtime analysis do not overturn the offline comparison; they show that architecture is only one layer of the final system story. The TCN is not preferred simply because it produced the most attractive number, but because it remained easier to defend once representation, policy, runtime interpretation, and deployment boundaries were considered together.

The report is therefore not claiming that graph-based temporal reasoning lacks value in fall detection as a field. It is claiming that, within this repository’s chosen representation, temporal contract, and policy layer, the custom GCN did not displace the TCN as the strongest defended baseline. That narrower statement is methodologically stronger because it stays tied to the conditions that were actually frozen and reviewed.

## Answer to RQ2

`RQ2` asked whether validation-side calibration and operating-point fitting materially influence practical alerting. The answer is yes. In the present system, deployable behaviour is governed by fitted operating-point profiles rather than by raw single-window probabilities alone. Alert behaviour is the product of calibration-aware fitting, temporal smoothing, `k/n` logic, cooldown behaviour, and persistence semantics rather than classifier discrimination alone.
...
The deeper lesson is that policy fitting changes the unit of success. Once the project is treated as a monitoring system rather than a ranking exercise, the key question becomes whether score streams can be converted into reviewable and operationally stable states. The report’s answer is yes: this conversion can be made explicit, validation-fitted, and auditable. That is stronger than merely observing that thresholds matter. It shows that the alert layer itself can be brought under artifact control and linked coherently to persistence, dashboard review, and caregiver-facing delivery.

## Answer to RQ3

`RQ3` asked what replay deployment evidence and limited realtime validation reveal about practical feasibility and runtime limits. The answer is mixed but still meaningful. The system is strong enough to support a bounded practical deployment claim under replay-oriented and live-demo conditions, but the current branch evidence also shows that runtime behaviour is highly profile-sensitive. The canonical replay summary, the 24-clip custom replay trace, and the live evidence pack collectively show that the monitor path is real and reviewable, while also showing that runtime success is not uniform across operating points or datasets and that the current uncertainty-aware path does not improve the bounded replay matrix.

The correct interpretation is practical feasibility in controlled settings rather than broad deployment closure. This captures the actual maturity level of the artifact. The project is neither a pure benchmark script nor a field-validated product. It occupies the middle ground of a research-led monitoring system whose strongest claims concern coherence, traceability, and bounded operational credibility. It is also important that the runtime evidence layers are not flattened into one headline score: the stronger historical deployment-oriented replay line and the weaker fixed replay failure-analysis line answer different questions and should not be read as contradictions. The subset-level kitchen weakness further reinforces that deployment readiness depends on camera placement and pose observability, not only on checkpoint quality and threshold selection.
...
These boundaries are not rhetorical devices. They are what make the system describable in layers without constant backtracking over hidden ambiguity. They also justify the long-report format. A compact paper can present the strongest numerical findings, but the full report can additionally show how those findings survived integration pressure, review, runtime interpretation, and freeze-state cleanup. In that sense, the report’s contribution is not only that it obtained a set of results, but that it turned a broad mixed project into a defendable technical artifact.
...
The project succeeded most clearly as a working research-and-systems artifact. Its strongest completed outcome is the integrated monitoring workflow: replay or realtime input can be processed through the frontend and backend, interpreted under an active model/profile, displayed in the monitor, and, for realtime incidents, persisted for later review. This directly satisfies the software-artifact requirement because the system demonstrates a meaningful end-to-end feature rather than merely describing one.

The project also succeeded in keeping model evidence and system evidence separate. The TCN result is defended through the frozen offline protocol and bounded replay evidence, while the deployed monitor is defended as a working prototype with explicit limits. This separation prevents the report from overstating what the demonstration proves.
...
The main lesson is that a fall-detection system cannot be judged only by a classifier score. The final artifact became stronger when the project controlled the full path from input capture to inference, policy interpretation, event persistence, review, and notification, while also stating clearly which parts remain bounded.
...
At the model-comparison layer, the strongest defended result is a cautious directional advantage for the TCN over the matched custom GCN under the frozen primary-dataset protocol. At the policy layer, the report shows that deployable behaviour depends not only on score ranking quality but also on how validation-fitted operating points, smoothing, cooldown, and persistence semantics shape alert interpretation. At the systems layer, the project demonstrates bounded end-to-end behaviour in which monitored input becomes interpreted runtime state, persisted incident history, and caregiver-facing notification under a reviewed path.
