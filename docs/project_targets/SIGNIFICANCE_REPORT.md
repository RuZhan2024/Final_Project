...
## Scope
Paired 5-seed significance analysis for final candidates (TCN vs GCN) using current stability artifacts.
...
## Hypotheses
1. H1: On CAUCAFall, TCN outperforms GCN on event metrics (`F1`, `Recall`) at matched seed settings.
2. H2: On LE2i, TCN outperforms GCN on event metrics (`F1`, `Recall`) at matched seed settings.
...
## Next Actions
1. Keep Wilcoxon as primary test in thesis text; report effect sizes and confidence intervals.
2. If stronger significance is required, increase seeds for top two candidates (e.g., `n=10`).
