...
# Support running this file directly (without pre-set PYTHONPATH).
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Optional dataset adapter mode: le2i|caucafall|muvim|urfall|urfd. "
             "When set, sequence loading is routed through fall_detection.data.adapters.",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--adapter_joint_layout",
        choices=["mp33", "internal17"],
        default="mp33",
        help="Adapter joint layout. Use mp33 for unified 33-joint pipeline; internal17 for legacy compatibility.",
    )
    ...
    if str(args.adapter_dataset).strip():
        ...
        adapter = build_adapter(
            ...
            joint_layout=str(args.adapter_joint_layout).strip().lower(),
        )
        print(f"[info] adapter enabled: dataset={adapter.dataset_name} joint_layout={adapter.joint_layout}")
    ...
    for stem in stems_ordered:
        ...
        if adapter is not None:
            seq = adapter.load_sequence(seq_path, fps_default=float(args.fps_default))
        ...
        for st, y, ov in chosen:
            ...
            if args.skip_existing and out_path.exists():
                continue
            # Defensive: ensure split directory exists even if cleaned concurrently.
