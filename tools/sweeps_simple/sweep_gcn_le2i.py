...
def main():
    ap = argparse.ArgumentParser(description="Grid/sampled sweep: GCN on LE2i (Makefile-driven).")
    ap.add_argument("--exp", default="gcn_le2i", help="Experiment name (used under outputs/sweeps).")
    ap.add_argument("--metric_key", default="monitor_score", help="history.jsonl key to maximize.")
    ap.add_argument("--max_runs", type=int, default=120, help="Deterministically sample this many runs (0 = full grid).")
    ...
    target = "train-gcn-le2i"
    save_dir_prefix = "outputs/le2i_gcn_fps_W48S12"
    ...
    mask_joint_ps = [0.10, 0.15, 0.20]   # NOTE: GCN uses MASK_* (global), not GCN_MASK_*
    ...
    for lr, do, gh, th, wd, ls, mj, mf, loss, fg, bs, pw in itertools.product(
        ...
        # pruning: avoid too many weird combos
        if bs == 0 and pw == "none":
            continue
    ...
    out = Path("outputs/sweeps") / "gcn" / "le2i" / args.exp
