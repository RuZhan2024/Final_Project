...
case "$mode" in
  torch-free)
    ...
    pytest -q \
      ...
      tests/test_datamodule_split_contract.py \
      tests/test_pose_preprocess_config.py \
      tests/test_data_pipeline_window_metadata.py \
      tests/test_data_pipeline_caucafall_label_fps.py \
      tests/server/test_notification_manager.py \
      tests/server/test_notifications_routes.py \
      tests/server/test_safe_guard_classifier.py \
      tests/server/test_safe_guard_store.py \
      tests/server/test_ai_report.py \
      tests/server/test_op_code_normalization.py \
      tests/server/test_events_test_fall_status_contract.py \
      tests/server/test_monitor_repository_event_status.py \
      tests/server/test_dashboard_repository_counts.py
    ;;
  contract)
    require_torch_importable
    pytest -q \
