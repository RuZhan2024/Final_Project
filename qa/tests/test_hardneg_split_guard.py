...
from fall_detection.training.graph_training_utils import _validate_hard_neg_paths as _validate_graph
...
def test_graph_hardneg_guard_rejects_nontrain_unknown_paths(tmp_path: Path) -> None:
    train_dir = tmp_path / "windows" / "train"
    p_train = _touch(train_dir / "ok.npz")
    p_other = _touch(tmp_path / "hardneg_pool" / "candidate.npz")

    _validate_graph([p_train], train_dir=str(train_dir), allow_nontrain=False)

    with pytest.raises(ValueError):
        _validate_graph([p_other], train_dir=str(train_dir), allow_nontrain=False)

    # Explicit override keeps backward compatibility for intentionally custom workflows.
    _validate_graph([p_other], train_dir=str(train_dir), allow_nontrain=True)
