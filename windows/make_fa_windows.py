...
def _meta_to_dict(meta_obj) -> Dict:
    """Best-effort conversion of meta payload to dict."""
    if meta_obj is None:
        return {}
    if isinstance(meta_obj, dict):
        return meta_obj
    if hasattr(meta_obj, "__dict__"):
        return dict(meta_obj.__dict__)
    ...
    try:
        with np.load(fp, allow_pickle=True) as z:
            ...
            if "meta" in z:
                ...
                meta = z["meta_json"].item() if z["meta_json"].shape == () else z["meta_json"][0]
            ...
            if y is None:
                # fallback to meta
                if "y" in md:
                    y = _safe_int(md.get("y"), default=None)
